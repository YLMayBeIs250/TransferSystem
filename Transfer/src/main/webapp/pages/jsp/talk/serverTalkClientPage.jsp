           ...
           if (r != null){
            ...
            TalkSub.onclick = function(){
                ...
                if(TalkWords.value == ""){                   
                	...
                	serverSay = '<div class="atalk"><span>商家 :'+ commonTime+'<br/>' + TalkWords.value +'</span></div>';
                	Words.innerHTML = Words.innerHTML + serverSay;               	
                	 var div = document.getElementById('words');
					 div.scrollTop = div.scrollHeight;
                	$.ajax({
                    	url : "<%=basePath %>Order_serverTalk",
     					type : "post",
     					dataType : "json",
     					data :  {
      					...
      					success : function(result){
      						if(result.json.state == 0){
    							...
    							for( var i = 0 ; i < result.json.list.length ; i++){
    								...
    								var  str = '<div class="btalk"><span>用户:'+commonTime +'<br/>' + result.json.list[i].message +'</span></div>' ;  
    								Words.innerHTML = Words.innerHTML + str; 
	...
	<div class="talk_con" id="talk_con">
		<div class="talk_show" id="words">
			<c:forEach items="${info }" var="a">
				<c:if test="${a.flag == '1' }">
					<div class="atalk">
						<span id="asay">商家:${a.sendTime }<br />${a.message}
						</span>
					</div>
				</c:if>
				<c:if test="${a.flag == '0' }">
					<div class="btalk">
						<span id="bsay">用户:${a.sendTime }<br />${a.message}
						</span>
					</div>
				</c:if>
